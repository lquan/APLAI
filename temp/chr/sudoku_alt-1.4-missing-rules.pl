...
cell(V,Cell), maybe(V,[T],1) <=> var(Cell) | Cell is T, cell(V,Cell).
...
%grmbl maybe(V,Cell,C) , ff(LL) <=> nonvar(Cell) | select(Cell2, Cell, R), cell(V,Cell2), C1 is C-1, maybe(V,R,C1).
...
9 4 3  6 8 2  1 7 x   (zitten 'x'n tussen!)
2 1 5  x x 7  9 4 6 
7 6 8  4 3 9  2 5 x

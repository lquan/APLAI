...
pp([X1,X2,X3|Rest],Var) :-
	
		
	

init2(P,Vars) :-
	length(P,N), N2 is N*N,
	flatten(P,P2),
	%length(P2,N2),				% rather a precond
	intsets(Vars,N,1,N2),		% make list of length 9 containing vars representing our numbers 1,..,9 with domain 1..81
	
	%initial values constraint
	( for(I,1,N2), foreach(El,P2), param(Vars) do
		( nonvar(El) -> 
			nth1(I2,Vars,Var), %select the corresponding number
			in(El,Var)		   %must be in set
		) 
	),
	
	( for(I,1,N), foreach(Var,Vars) do
		(for(I2,1,N2), foreach(X,Bools), param(Vars) do
			(
				in(I2,Var,X)	
			)
...
init(P,Vars):-
	N is 9, N2 is N*N,
	length(P,N),    			%input must be 9 list of lists, i.e. standard sudoku
	flatten(P, P2),
	intsets(Vars,N,1,N2),		%make list of length 9, represent our numbers 1,..,9 with domain 1..81
	
	(for(I,1,N2), foreach(El,P2) do
		assert(cell(I,El)) 		%initial values	
	),
	%Array =.. [[]|P2],
	...
	( for(I,1,N), foreach(Var,Vars), param(N) do
		...
		( foreach(J,L), param(Var) do
			in(J,Var) 					 % power set of {1..81} with already filled in
			 							 % ic_sets:(Var :: L .. [1..81]),  %might be better
		),	
		#(Var,N),			 			 % cardinality	
...
solve(P,Vars) :-				
	init2(P,Vars),
	label_sets(Vars),
	(for(I2,1,9), foreach(Var,Vars) do
		( for(I,1,81), param(Var,I2) do
			in(I,Var,Bool),
			( (Bool =:= 1 -> cell(I,I2) ; true))
		)
	),
	find_cells(Cells),
...
% find cells and print their contents. ??how map this to list of lists as given?
find_cells(L) :-
	findall(cell(Id,V), cell(Id,V), L),
	(foreach(Cell,L) do
		writeln(Cell)
	).

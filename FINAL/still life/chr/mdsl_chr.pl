...
/* A living cell should have 2 of 3 living neighbours */
cell(X,1), nbs(X,XNB) ==> sumgeq(XNB,2), sumleq(XNB,3).
/* A dead cell should not have 3 living neighbours */
cell(X,0), nbs(X,XNB) ==> sumneq(XNB,3).

% some more chr rules to deal a.o. with sumgeq/2, ...
/* If there is a constraint where <I> only has one neighbour <X>, while an other 'neighbour constraint'
 * exist for <I> which has one or more neighbours, and <X> is not part of the second constraint. Then both constraints
 * are replace with a new constraints where the first and the second constraints are combined. */
nbs(I,[X]), nbs(I,L) <=> \+ member(X,L) | nbs(I,[X|L]).

/* Propagation rules used to build up neighbours */
...
/* Two cells with the same cellID have the same value */
cell(X,V1) \ cell(X,V2) <=> V1 = V2.

/* Cleanup of constraints where the meaning is alike, keeping the stronger version. */
sumgeq(L,S1) \ sumgeq(L,S2) <=> S1 >= S2 | true.
sumleq(L,S1) \ sumleq(L,S2) <=> S1 =< S2 | true.
sumneq(L,S) \ sumneq(L,S) <=> true.

/* Recursively, we check the value of the list when we know the value of a particular cell. */
% we can directly reject when the length is smaller
sumgeq(L,S) <=> length(L,L1), S > L1 | fail.
sumgeq([],S) <=> S =< 0.
cell(I,Val) \ sumgeq(L,S) <=> select(I,L,Ls) | S2 is S - Val, sumgeq(Ls,S2).

/* Anolog to previous constraint */
sumleq([],S) <=> S >= 0.
